...
########################################################################################################################
    ...
    def CreateGraphValue(self):
        ...
        ArrOy_plus = [x * self.MaxError for x in self.ListValue]
        ArrOy_minus = [x * self.MinError for x in self.ListValue]
...
########################################################################################################################
    ...
    def CallMessengerChoices(self, result, value) -> str:
        ...
        error_message += f'{MFRT.ConvertFloatToString(self.MinError * value if self.MinError * value > 0 else 0, resolution=3)} < '
