...
if language == 'English(default)':
    ...
    Moko.Plugin('MOKO ExPlugin', 'set', 'Number2=14')
    ...
    time.sleep(10)
    Moko.Plugin('MOKO ExPlugin', 'set', 'ShowTab=Graph')
    time.sleep(3)
    ...
    Moko.Plugin('MOKO ExPlugin', 'set', 'Graph=stop')
    ...
    Moko.Messenger('set', 'Set Number1|2', 'Команда *Number1|2* задает число Number1 или Number2 в окне Main. '
                                           'Для этого после команды ставиться = и число, которое нужно записать. ' 
    ...
    Moko.Report("explugin", 'set', 'string', a)
    Moko.Messenger('set', 'Set Screenshot', 'Команда *Screenshot* делает скриншот плагина, '
                                            'название которого состоит из '
                                            'даты и времени в момент скриншота. Скриншот сохраняется в отдельную папку '
                                            'App/screenshots в корневом каталоге ExPlugin.')
    Moko.Plugin('MOKO ExPlugin', 'set', 'Screenshot')
    Moko.Messenger('set', 'Set ChangeLedLoop', 'Команда *ChangeLedLoop* меняет значение индикатора '
                                               'Led Loop в окне Main.')
    Moko.Plugin('MOKO ExPlugin', 'set', 'ChangeLedLoop')

    Moko.Messenger('set', 'Set ShowTab', 'Команда *Showtab* меняте отображает нужное окно плагина '
                                         '(Main | Graph | Info). Для этого после команды ставиться = '
                                         'и название нужного окна.')
    Moko.Plugin('MOKO ExPlugin', 'set', 'ShowTab=Info')
    time.sleep(10)
    Moko.Plugin('MOKO ExPlugin', 'set', 'ShowTab=Graph')
    time.sleep(3)
    Moko.Messenger('set', 'Set Graph', 'Команда *Graph* запускает или останавливает график в окне Graph. '
                                       'Для старта после команды *Graph* ставиться =start, а для остановки =stop.')
    Moko.Plugin('MOKO ExPlugin', 'set', 'Graph=start')
