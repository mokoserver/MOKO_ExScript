...
if language == 'Russian':
    Moko.Messenger('set', 'Script message', 'В текущем скрипте в тестовом протоколе данные записываются в виде строки по определенным закладкам.')
    Moko.Messenger('set', 'Report mode', 'Функция Report имеет 2 типа: *set* и *get*. Для записи каких-либо данных в протокол используется тип *set*'
                              'Тип *get* используется в случае получения данных из протокола.')
    Moko.Messenger('set', 'Report name', 'Report name - параметр функции, в котором задаётся название закладки, определённой в Word-протоколе.')
    Moko.Messenger('set', 'Report kind', 'Report kind - параметр, определяющий в каком виде записывать данные в протокол (*string*, *table*).')
